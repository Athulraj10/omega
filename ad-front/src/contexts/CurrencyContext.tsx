...
export const CurrencyProvider: React.FC<CurrencyProviderProps> = ({ children }) => {
  ...
  const fetchCurrencies = async () => {
    try {
      ...
      if (err.response?.status === 404 || err.code === 'ERR_NETWORK' || err.message?.includes('fetch')) {
        console.log('🌐 Backend server not available, using fallback currencies');
        setError('Backend server not available. Using default currencies.');
      } else {
