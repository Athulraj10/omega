...
function* fetchUsersSaga(action: any): Generator<any, void, any> {
  try {
    ...
    // The backend sends data nested under response.data.data
    const payload = {
    ...
    console.log('Users count in payload:', payload.users.length);
    yield put({ type: FETCH_USERS_SUCCESS, payload });
  } catch (error: any) {
    console.error('Error in fetchUsersSaga:', error);
...
function* updateUserStatusSaga(action: any): Generator<any, void, any> {
  try {
    ...
    if (response.data && response.data.data) {
      console.log('📋 Response has data structure:', response.data.data);
    }
    ...
    if (typeof window !== 'undefined') {
      const { notifySuccess } = yield import('@/utils/helper');
    ...
    // Show error message
    if (typeof window !== 'undefined') {
      const { notifyDanger } = yield import('@/utils/helper');
      notifyDanger(error.response?.data?.message || 'Failed to update user status');
    }
  }
}
