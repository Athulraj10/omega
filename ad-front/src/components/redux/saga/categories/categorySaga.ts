...
// Temporarily define the missing action types locally
const FETCH_CATEGORIES_FOR_PRODUCT_REQUEST = 'categories/fetchCategoriesForProductRequest';
const FETCH_CATEGORIES_FOR_PRODUCT_SUCCESS = 'categories/fetchCategoriesForProductSuccess';
const FETCH_CATEGORIES_FOR_PRODUCT_FAILURE = 'categories/fetchCategoriesForProductFailure';

const fetchCategoriesForProductSuccess = (data: any) => ({ type: FETCH_CATEGORIES_FOR_PRODUCT_SUCCESS, payload: data });
const fetchCategoriesForProductFailure = (error: string) => ({ type: FETCH_CATEGORIES_FOR_PRODUCT_FAILURE, payload: error });
...
function* fetchCategoriesForProductSaga(): Generator<any, void, any> {
  try {
    ...
    if (response.data.success) {
      ...
      // Use mainCategories array from the response
      const categoriesData = response.data.data.mainCategories || [];
      console.log('📋 Categories data to dispatch:', categoriesData);
      yield put(fetchCategoriesForProductSuccess(categoriesData));
    } else {
