...
const userReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(updateUserStatusSuccess, (state, action) => {
      state.loading = false;
      console.log('🔄 updateUserStatusSuccess reducer called with payload:', action.payload);
      ...
      if (action.payload && typeof action.payload === 'object') {
        ...
        } else if (payload.data && payload.data.user && payload.data.user.id) {
        ...
        if (updatedUser) {
          ...
          if (userIndex !== -1 && state.users[userIndex]) {
            const oldStatus = state.users[userIndex].status;
            state.users[userIndex] = {
              ...state.users[userIndex],
              status: updatedUser.status
            };
            console.log('✅ User status updated:', { userId: updatedUser.id, oldStatus, newStatus: updatedUser.status });
            console.log('🔄 Updated user object:', state.users[userIndex]);
          } else {
            console.warn('⚠️ User not found in state for update:', updatedUser.id);
            console.log('🔍 Available users:', state.users.map(u => ({ id: u.id, name: u.name })));
          }
        } else {
          console.warn('⚠️ Invalid payload structure for user status update:', payload);
        }
      }
    })
