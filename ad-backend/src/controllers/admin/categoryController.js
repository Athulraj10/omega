...
// Get categories for product assignment (hierarchical structure)
const getCategoriesForProduct = async (req, res) => {
  try {
    ...
    const categoriesForProduct = mainCategories.map(mainCategory => ({
      id: mainCategory._id,
      name: mainCategory.name,
      description: mainCategory.description,
      icon: mainCategory.icon,
      slug: mainCategory.slug,
      type: 'main',
      subcategories: mainCategory.subcategories.map(sub => ({
        id: sub._id,
        name: sub.name,
        description: sub.description,
        icon: sub.icon,
    ...
    // Also include standalone subcategories (those without main category)
    ...
    const standaloneCategories = standaloneSubcategories
      .filter(sub => !mainCategories.some(main => 
        main.subcategories.some(subcat => subcat._id.toString() === sub._id.toString())
      ))
      .map(sub => ({
        id: sub._id,
        name: sub.name,
        ...
        slug: sub.slug,
        type: 'sub',
        parentId: sub.parentCategory?._id
      }));

    return Response.success(res, {
      mainCategories: categoriesForProduct,
    ...
    return Response.error(res, 'Failed to fetch categories for product', 500);
  }
};
