...
module.exports = {
  ...
  deleteHeroSlider: async (req, res) => {
    try {
      ...
      // Soft delete
      await slider.softDelete(req.authAdminId);
  ...
  toggleHeroSliderStatus: async (req, res) => {
    try {
      ...
      slider.updatedBy = req.authAdminId;
      await slider.save();
  ...
  duplicateHeroSlider: async (req, res) => {
    try {
      ...
      const duplicatedSlider = await slider.duplicate(req.authAdminId);
  ...
  bulkOperations: async (req, res) => {
    try {
      ...
      switch (operation) {
        case 'activate':
          updateData = { status: 'active', updatedBy: req.authAdminId };
          message = "Sliders activated successfully";
          break;
        case 'deactivate':
          updateData = { status: 'inactive', updatedBy: req.authAdminId };
          message = "Sliders deactivated successfully";
          break;
        case 'delete':
          // Soft delete
          const sliders = await HeroSlider.find({ _id: { $in: ids } });
          await Promise.all(sliders.map(slider => slider.softDelete(req.authAdminId)));
          message = "Sliders deleted successfully";
          break;
        case 'update':
          updateData = { ...data, updatedBy: req.authAdminId };
